...
function _renderToStringWithData(component) {
  var startGetDataFromTree = process.hrtime()
  return require('react-apollo').getDataFromTree(component).then(() => {
    var endGetDataFromTree = process.hrtime(startGetDataFromTree)

    var startRenderToString = process.hrtime()
    var markup = require('react-dom/server').renderToString(component)
    var endRenderToString = process.hrtime(startRenderToString)
    return {
      markup,
      renderTimeMetric: {
...
const render = name => {
  ...
  const root = (
    <AppContainer>
      <RenderProvider history={isPage && !customRouting && createHistory()}>
        {!isPage && <ExtensionPoint id={name} />}
      </RenderProvider>
    </AppContainer>
  )
  return canUseDOM
